...
# 【修改点1】引入网络代理接口，而非直接数据库接口
# 所有的增删改查现在都通过网络发送给 Member B 的服务器
...
class QuestionWidget(tk.Frame):
    ...
    def __init__(self, master, question_data, survey_id, refresh_callback, checker, sock):
        ...
        self.sock = sock  # 【修改点3】存储 socket 对象，用于后续网络请求
    ...
    def _refocus(self):
        """辅助方法：强制将窗口拉回最顶层（解决弹窗关闭后 Dashboard 置顶的问题）"""
    ...
    def get_type_name(self):
        """获取题型的中文显示名称"""
        type_map = {
            "choice": "单选题", "radio": "单选题",
            "checkbox": "多选题", "text": "填空题", "slider": "滑动条"
        }
    ...
    def render_slider_preview(self):
        ...
        scale = tk.Scale(frame, from_=1, to=10, orient=tk.HORIZONTAL, bg="white", length=300, showvalue=0)
        scale.set(5)
    ...
    def save_title(self, event=None):
        ...
        if is_v:
            ...
            return
        # 【修改点5】网络调用：更新题目文本
        update_question_text(self.sock, self.q_id, new_text)
    ...
    def save_option(self, option_id, new_text):
        ...
        if is_v:
            ...
            return
        # 【修改点6】网络调用：更新选项文本
        update_option_text(self.sock, option_id, new_text)

    def add_new_option(self):
        count = len(self.options_container.winfo_children()) + 1
        text = f"选项{count}"
        # 【修改点7】网络调用：添加新选项
        new_id = add_option(self.sock, self.q_id, count, text)
...
class SurveyEditorWindow(tk.Toplevel):
    ...
    def __init__(self, master, user_id, sock):
        ...
        self.title("编辑调查")
        self.sock = sock  # 【修改点12】存储 socket 对象
    ...
    def show_editor_page(self):
        ...
        finish_btn = tk.Button(top_bar, text="完成编辑", bg="#4CAF50", fg="white", font=("Arial", 12, "bold"),
                               relief="flat", padx=15, cursor="hand2", command=self.finish_editing)
        ...
        self.main_title_entry = tk.Entry(title_frame, font=("Arial", 18, "bold"), fg="#2196F3", bg="white", bd=0,
                                         justify="center")
        ...
        # 基础题型
        tk.Label(right_panel, text="添加基础题型", font=("Arial", 10), fg="gray", bg="white").pack(anchor="w", padx=60)
    ...
    def update_survey_title_action(self, event):
        ...
        if is_v:
            ...
            return
        # 【修改点15】网络调用
        update_survey_title(self.sock, self.survey_id, new_title)
    ...
    def render_questions(self, scroll_action=None):
        ...
        for q in questions:
            # 【修改点17】传递 sock 给 QuestionWidget
    ...
    def add_question_directly(self, q_type):
        ...
        default_titles = {
            ...
            "text": "填空题", "slider": "评分题 (1-10)"
        }
        q_id = add_question(self.sock, self.survey_id, new_index, default_titles[q_type], q_type)

        if q_type in ["choice", "checkbox"]:
            add_option(self.sock, q_id, 1, "选项1")
            add_option(self.sock, q_id, 2, "选项2")
        elif q_type == "slider":
            for i in range(1, 11):
                add_option(self.sock, q_id, i, str(i))
    ...
    def add_template_directly(self, tpl_type):
        ...
        templates = {
            "tpl_name": {"title": "您的姓名是？", "type": "text", "options": []},
            "tpl_gender": {"title": "您的性别？", "type": "choice", "options": ["女", "男", "其他"]},
            "tpl_age": {"title": "您的年龄段：", "type": "choice",
                        "options": ["18岁以下", "18~25", "26~30", "31~40", "41~50", "51~60", "60以上"]},
            "tpl_mobile": {"title": "请输入您的电话号码：", "type": "text", "options": []}
        }
        tpl = templates.get(tpl_type)
        if not tpl: return

        q_id = add_question(self.sock, self.survey_id, new_index, tpl['title'], tpl['type'])
        for idx, opt_text in enumerate(tpl['options'], 1):
            add_option(self.sock, q_id, idx, opt_text)

        self.render_questions(scroll_action='bottom')

    def finish_editing(self):
        self.update_survey_title_action(None)
        if messagebox.askyesno("发布问卷", "是否将问卷立即发布？\n（是：发布 [Active]，否：保存为草稿 [Draft]）",
                               parent=self):
            try:
                # 【修改点20】网络调用
